#!/usr/bin/env bash
# HDDm-zikplayer ana kurulum betiği (Raspberry Pi OS, Bullseye veya Bookworm).
#
#   sudo ./install/install.sh
#
# Yaptıkları:
#   * paketleri kurar (mpd, python bağımlılıkları)
#   * zikplayer sistem kullanıcısını ve veri dizinlerini oluşturur
...
SERVICE_USER="zikplayer"
VENV_DIR="/opt/zikplayer/venv"
DATA_DIR="/var/lib/zikplayer"
ETC_DIR="/etc/zikplayer"
...
install -d -m 0755 /opt/zikplayer
...
if [[ ! -f "$ETC_DIR/zikplayer.env" ]]; then
  sed -e "s|@MUSIC_ROOT@|$MUSIC_ROOT|g" \
      -e "s|@VOLUME_MODE@|$VOLUME_MODE|g" \
      "$CONFIG_DIR/zikplayer.env" > "$ETC_DIR/zikplayer.env"
else
  log "  $ETC_DIR/zikplayer.env zaten var, dokunulmadı"
fi
chmod 0644 "$ETC_DIR/zikplayer.env"

# MPD yapılandırması. Mevcut dosya yedeklenir.
if [[ -f /etc/mpd.conf && ! -f /etc/mpd.conf.zikplayer-bak ]]; then
  cp /etc/mpd.conf /etc/mpd.conf.zikplayer-bak
  log "  eski /etc/mpd.conf -> /etc/mpd.conf.zikplayer-bak"
fi
...
install -m 0440 "$CONFIG_DIR/zikplayer-sudoers" /etc/sudoers.d/zikplayer
visudo -cf /etc/sudoers.d/zikplayer >/dev/null

# --- systemd -----------------------------------------------------------------

log "Servisler kuruluyor"
for unit in zikplayer-api zikplayer-panel; do
...
$(log "Kurulum tamamlandı")
  ...
  Ayarlar     : $ETC_DIR/zikplayer.env
  Günlükler   : journalctl -u zikplayer-api -f
